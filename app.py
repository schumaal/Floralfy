...
def load_classifier():
    return pipeline(
        ...
        model="microsoft/resnet-50",   # gut für Pflanzen & schnell
        device=-1
    )
...
with tab3:
    ...
    new_info = st.text_area("Infos (eine Zeile pro Punkt)")
...
with st.sidebar:
    st.markdown("### Modell")
    st.write("Verwendet: **ResNet-50** (stabil & schnell)")

st.caption("🌱 Viel Freude beim Entdecken der Pflanzenwelt!")import streamlit as st
from PIL import Image
from transformers import pipeline
import io
